...
export const calculateSubjectGrade = (studentId, subjectId, subjects, grades) => {
...
export const generateReportCard = (studentId, reportPeriod, comments, students, subjects, grades) => {
    ...
    const studentSubjectIds = [...new Set(grades
            ...
            .map(g => g.subjectId))].filter((id) => Boolean(id));
...
export const getSubjectCalculationBreakdown = (studentId, subjectId, subjects, grades) => {
    const subject = subjects.find(s => s.id === subjectId);
    if (!subject)
        return null;
